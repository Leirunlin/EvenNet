...
class Even_prop(MessagePassing):
    def __init__(self, K, alpha, Init, Gamma=None, bias=True, **kwargs):
        ...
        if Init == 'SGC':
            ...
            # PPR-like
            TEMP = alpha * (1 - alpha) ** (2*np.arange(self.K + 1))
...
class EvenNet(torch.nn.Module):
    def __init__(self, in_channels, out_channels, hidden_size, K, alpha, Init, dprate, dropout, num_layers, use_ln, layer_norm_first):
        super(EvenNet, self).__init__()
        self.mlp = MLP(in_channels=in_channels, hidden_channels=hidden_size, out_channels=out_channels,
                       dropout=dropout, use_ln=use_ln, layer_norm_first=layer_norm_first, num_layers=num_layers)
