...
def main():
    ...
    if not args.batch_eval:
        ...
        if args.dataset.lower() in ["arxiv", "grb-aminer", "grb-reddit"] and not args.eval_target:
            ...
            args.batch_attacks = ["rnd", "pgd", "gia", "seqgia", "rseqgia", "metagia", "rmetagia", "tdgia", "rtdgia",
                                  "speitml", "atdgia", "ratdgia", "seqagia", "seqragia"]
            report_batch = ["pgd", "gia", "seqgia", "rseqgia", "metagia", "rmetagia", "tdgia", "rtdgia", "speitml",
                            "atdgia", "ratdgia", "seqagia", "seqragia"]
        elif args.eval_target:
            # targeted attack baselines
            args.batch_attacks = ["vanilla", "rnd", "pgd", "gia", "seqpgd", "rseqpgd", "seqgia", "rseqgia", "metagia",
                                  "rmetagia", "tdgia", "rtdgia", "speitml", "atdgia", "ratdgia", "agia", "ragia",
                                  "seqagia", "seqragia"]
            report_batch = ["vanilla", "pgd", "gia", "seqgia", "rseqgia", "metagia", "rmetagia", "tdgia", "rtdgia",
                            "speitml", "atdgia", "ratdgia", "agia", "ragia", "seqagia", "seqragia"]
        else:
            # non-target small graphs
            args.batch_attacks = ["rnd", "pgd", "gia", "seqgia", "rseqgia", "metagia", "rmetagia", "tdgia", "rtdgia",
                                  "speitml", "atdgia", "ratdgia", "agia", "ragia", "seqagia", "seqragia"]
            report_batch = ["pgd", "gia", "seqgia", "rseqgia", "metagia", "rmetagia", "tdgia", "rtdgia", "speitml",
                            "atdgia", "ratdgia", "agia", "ragia", "seqagia", "seqragia"]
    ...
    if args.model.lower() == "sage":
        ...
        model = MLP(data.num_features, args.hidden_channels,
                    ...
                    args.dropout, layer_norm_first=args.layer_norm_first,
                    use_ln=args.use_ln)
    elif args.model.lower() == 'evennet':
        model = EvenNet(data.num_features, num_classes,
                        args.hidden_channels, K=args.K, alpha=args.alpha, Init='PPR', dropout=args.dropout,
                        dprate=args.dropout, num_layers=args.num_layers, layer_norm_first=args.layer_norm_first,
                        use_ln=args.use_ln)
    elif args.model.lower() == 'gprgnn':
        model = GPRGNN(data.num_features, num_classes,
                       args.hidden_channels, K=args.K, alpha=args.alpha, Init='PPR', dropout=args.dropout,
                       dprate=args.dropout, num_layers=args.num_layers, layer_norm_first=args.layer_norm_first,
                       use_ln=args.use_ln)
    ...
    # print("-----Split Info: ", len(train_idx), len(val_idx), len(test_idx), "------")
    data = data.to(device)
    if args.inductive:
    ...
    for run in range(args.runs):
        ...
        if args.eval_robo and not args.batch_eval:
            ...
            target_idx = test_idx
            x_attack, adj_attack, target_idx = eval_robustness(model, x_test, adj_test, target_idx, data.y, device,
                                                               args, run)
            ...
            for (i, atk) in enumerate(args.batch_attacks):
                for j in range(max(args.mul_run, 1)):
                    ...
                    x_attack, adj_attack, target_idx = eval_robustness(model, x_test, adj_test, target_idx, data.y,
                                                                       device, args, run=j)
    ...
    if args.eval_robo and not args.batch_eval:
        ...
        for (i, atk) in enumerate(args.batch_attacks):
            print(
                f"Average test robustness accuracy under {atk}: {np.mean(batch_robo_tests[atk])} ± {np.std(batch_robo_tests[atk])}")
